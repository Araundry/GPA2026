...
try:
    while True:
        ...
        # === 🧹 ここから絶対に壊れない全自動お掃除・送信コード ===
        repo_dir = "/home/pi/Desktop/GPA2026"
        github_url = "https://github.com/Araundry/GPA2026.git"

        try:
            # 1. 普通にGitにデータを追加して保存を試みる（エラーを隠さず検知する）

            subprocess.run(["git", "add", "."], cwd=repo_dir)
            subprocess.run(["git", "commit", "-m", "Auto Update"], cwd=repo_dir)
            subprocess.run(["git", "push", "origin", "main", "--force"], cwd=repo_dir, check=True)

        except subprocess.CalledProcessError:


        # === 🧹 あなたの仕様（ゴミが残ったら安全にお掃除）を完璧に叶えるコード ===
            repo_dir = "/home/pi/Desktop/GPA2026"

            try:
                # 1. あなたの狙い通り、data.json だけをピンポイントで追加して送信
            ...
            # 2. Gitが少しでも失敗したら、ここが確実に発動してお掃除します！
                print("⚠️ Gitの破損またはエラーを検知しました！全自動でお掃除・修復します...")
                try:
                    import shutil
                    import os
                    ...
                    if os.path.exists(git_dir):
                        shutil.rmtree(git_dir)  # 壊れた.gitをまるごと削除（プログラムやjsonは消えません）
                    
                    # ゼロから一瞬でアルバムを繋ぎ直す
                    subprocess.run(["git", "init"], cwd=repo_dir)
    
                    subprocess.run(["git", "remote", "add", "origin", github_url], cwd=repo_dir)
    
                    subprocess.run(["git", "remote", "add", "origin", github_url], cwd=repo_dir)
    
                    subprocess.run(["git", "remote", "set-url", "origin", github_url], cwd=repo_dir)
                    subprocess.run(["git", "remote", "set-url", "origin", github_url], cwd=repo_dir)
                    subprocess.run(["git", "add", "."], cwd=repo_dir)
                    subprocess.run(["git", "commit", "-m", "Reset and Clean Update"], cwd=repo_dir)
                    subprocess.run(["git", "branch", "-M", "main"], cwd=repo_dir)
                    subprocess.run(["git", "push", "-u", "origin", "main", "--force"], cwd=repo_dir)
                    
                    print("🧹 全自動お掃除が完了しました！そのまま1分更新を継続します。")
                except Exception as repair_error:
                    print(f"❌ お掃除中にエラーが発生しました（ネットワーク切れなど）: {repair_error}")


            # 2. 送信中に電源を切るなどして、通路にゴミ（エラー）が残った場合
            print("⚠️ 通路に未送信のゴミを検知しました。安全にお掃除・同期します...")
            try:
                # 💡 家を壊さず、通路に残った中途半端なゴミ（コミット）だけを安全にリセットする魔法のコマンド
                subprocess.run(["git", "reset", "--mixed", "origin/main"], cwd=repo_dir)
                # 💡 GitHub側の最新の歴史をおとなしく引き込んで、手元と綺麗にガッチャンコ（同期）させる
                subprocess.run(["git", "pull", "-u", "origin", "main", "--force"], cwd=repo_dir)
                print("🧹 通路のゴミ掃除が安全に完了しました！")
            except:
                pass
